...
def getPlots(patient, sop, k, c_value, seizures):
    # get classifier time plots
    [datetimes, labels, alarms, firing_power_output,
     vigilance, vigilance_datetimes] = testOnePatientGetPlots(patient, sop, k, c_value, seizures)
    ...
    for i in range(seizures - 3):
        ...
        # plt.show()
        fig.savefig("Patient" + str(patient) + "Seizure" + str(i + 3 + 1) + "SVM.png", dpi=200, bbox_inches='tight')
        fig.savefig("Patient" + str(patient) + "Seizure" + str(i + 3 + 1) + "SVM.pdf", dpi=200, bbox_inches='tight')
