...
from test_OnePatient_getPlots_SNNs import testOnePatientGetPlots


def getPlots(patient, sop, seizures):
    # get classifier time plots
    [datetimes, labels, alarms, firing_power_output,
     vigilance, vigilance_datetimes] = testOnePatientGetPlots(patient, sop, seizures)
    ...
    for i in range(seizures - 3):
        ...
        # plt.show()
        fig.savefig("Patient" + str(patient) + "Seizure" + str(i + 3 + 1) + "SNN.png", dpi=200, bbox_inches='tight')
        fig.savefig("Patient" + str(patient) + "Seizure" + str(i + 3 + 1) + "SNN.pdf", dpi=200, bbox_inches='tight')
